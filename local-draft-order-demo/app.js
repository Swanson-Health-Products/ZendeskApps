...
async function apiGet(path) {
  const url = `${els.apiBase.value.trim().replace(/\/$/, '')}${path}`;
    ...
    openBtn.addEventListener('click', async () => {
      try {
        ...
        orderItems = lineEdges.map(({ node }) => ({
          ...
          title: node.title || node.name || node.variant?.product?.title || node.variant?.title || '',
...
if (els.apiKey) {
  const savedKey = localStorage.getItem('shopify_demo_api_key') || '';
  if (savedKey && !els.apiKey.value) {
    els.apiKey.value = savedKey;
  }
  els.apiKey.addEventListener('change', () => {
    localStorage.setItem('shopify_demo_api_key', els.apiKey.value.trim());
  });
}

if (els.apiBase) {
  const savedBase = localStorage.getItem('shopify_demo_api_base') || '';
  if (savedBase && !els.apiBase.value) {
    els.apiBase.value = savedBase;
  }
  els.apiBase.addEventListener('change', () => {
    localStorage.setItem('shopify_demo_api_base', els.apiBase.value.trim());
  });
}
...
els.btnSearchCustomer.addEventListener('click', async () => {
  try {
...
setStatus(els.apiStatus, 'Enter your API key, then run the steps in order.', '');
setInvoiceUrl('');
setTotals(null);
setDraftButtonState(false);
